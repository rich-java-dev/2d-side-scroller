...
class Ghost extends Enemy {
    ...
    public constructor(x: number, y: number, vx: number, drop: Key | null) {
        ...
        super(3, x, y, 70, 74, "", direction, vx, drop)
    ...
    public update = () => {
        ...
        this.y = this.y + this.vy + this.direction * 15 * Math.sin(this.x * Math.PI / 100);
    ...
    public draw = (ctx: any, offsetX: number, offsetY: number) => {
        ...
        if (this.hp <= 0) {
            ctx.fillStyle = Patterns.getDeadGhostPattern(ctx)
            ctx.translate(0, 70)
            ctx.fillRect(0, 0, this.height+5, this.width+5)
            ctx.restore()
        }
        else {
            ctx.fillStyle = Patterns.getGhostPattern(ctx)
